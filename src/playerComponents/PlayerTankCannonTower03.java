...
public class PlayerTankCannonTower03 extends TankCannonTower {

	public PlayerTankCannonTower03(Tank tank) {
		super(tank);
	...
	public void create() {
		...
		Point2D firstPoint = new Point2D(cp.x() - splitX, cp.y() - 1.5 * splitY);
		towerPoint.add(firstPoint);
		Point2D secondPoint = new Point2D(cp.x() - splitX, cp.y() - 0.5
				* splitY);
		towerPoint.add(secondPoint);
		Point2D thirdPoint = new Point2D(cp.x() - 0.3 * splitX, cp.y() + splitY);
		towerPoint.add(thirdPoint);
		Point2D fourthPoint = new Point2D(cp.x() - 0.3 * splitX, cp.y() + 5.5
				* splitY);
		towerPoint.add(fourthPoint);
		Point2D fifthPoint = new Point2D(cp.x() + 0.3 * splitX, cp.y() + 5.5
				* splitY);
		towerPoint.add(fifthPoint);
		Point2D sixPoint = new Point2D(cp.x() + 0.3 * splitX, cp.y() + splitY);
		towerPoint.add(sixPoint);
		Point2D sevenPoint = new Point2D(cp.x() + splitX, cp.y() - 0.5 * splitY);
