...
public class PlayerTankAction04 extends TankAction {
	...
	public PlayerTankAction04(PublicResource pr, Map mapInfo) {
		super(pr, mapInfo);
	...
	public void body_action() {
		...
		if ((isBounded(x + tankDisX, y + tankDisY)))
			tankBehaviour.setSpeed(-v0x, -v0y, boundRotate);
	...
	public void tower_action() {
		...
		if (cpDraw.mouseClicked() && cpDraw.mouseClickedCount() == 1) {
			double x = cpDraw.mouseX();
			double y = cpDraw.mouseY();
			findAngle = Math.PI / 2
					- Math.atan2((y - tankBody.cp.y()), (x - tankBody.cp.x()));
